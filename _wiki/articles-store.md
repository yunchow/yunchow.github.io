---
layout: wiki
title: 文章收录
categories: wiki
description: 文章收录
keywords: 文章收录
---

## JVM
* [Java 垃圾回收手册](https://plumbr.eu/java-garbage-collection-handbook)
* [理解 CMS GC 日志](http://www.cnblogs.com/kakaxisir/p/6209266.html)

## Scala
* [Scala学习资料大全](https://stackoverflow.com/tags/scala/info)
* [我为什么想并且要学习 Scala](https://codingstyle.cn/topics/134?hmsr=toutiao.io&utm_medium=toutiao.io&utm_source=toutiao.io)
* [Scala 课堂!](https://twitter.github.io/scala_school/zh_cn/)
* [Scala Akka 教程](https://doc.yonyoucloud.com/doc/akka-doc-cn/2.3.6/scala/book/index.html)

## 中间件
* [RPC 框架 Dubbox](http://dangdangdotcom.github.io/dubbox/)
* [分布式数据层](http://shardingjdbc.io/1.x/docs/00-overview/intro/)
* [分布式任务高度 Elastic-Job-Lite](http://dangdangdotcom.github.io/elastic-job/elastic-job-lite/00-overview/)
* [基于 Dubbo 框架构建分布式服务](http://shiyanjun.cn/archives/1075.html)
* [堆外缓存 Mapdb](https://jankotek.gitbooks.io/mapdb/content/)

## 高并发
* [Java 并发课程](http://docs.oracle.com/javase/tutorial/essential/concurrency/index.html)
* [10 分钟了解 Actor 模型](http://www.brianstorti.com/the-actor-model/)

## 架构
* [架构蓝图--软件架构『4+1』视图模型](https://www.ibm.com/developerworks/cn/rational/r-4p1-view/)

## IOS
* [WebApp](https://developer.apple.com/library/content/documentation/AppleApplications/Reference/SafariWebContent/ConfiguringWebApplications/ConfiguringWebApplications.html#//apple_ref/doc/uid/TP40002051-CH3-SW4)

## 其他
* [使用 Maven enforcer 插件检查依赖冲突](http://www.oolap.com/2015-10-22-use-maven-enforcer-plugin-check-deps)

